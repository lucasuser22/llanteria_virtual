		...
		public function loginUser() {
			if ($_POST) {
				if (isset($_POST['txtEmail']) && isset($_POST['txtPassword'])) {
					...
					// Lógica para inicio de sesión con correo solamente
					$strUsuario = strtolower(strClean($_POST['email']));
					$requestUser = $this->model->loginEmail($strUsuario);
		
					if (empty($requestUser)) {
						$arrResponse = array('status' => false, 'msg' => 'El usuario no está registrado.');
					} else {
						$arrData = $requestUser;
						if ($arrData['status'] == 1) {
							$_SESSION['idUser'] = $arrData['idpersona'];
